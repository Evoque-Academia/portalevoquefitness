...
def _horas_uteis_com_pausas(inicio: datetime, fim: datetime, pausas: List[Tuple], now: Optional[datetime] = None) -> Tuple[float, float]:
    ...
    for p_ini, p_fim in pausas:
        # Pausa ainda aberta: usa o instante fixo passado pelo chamador
...
class ServicoMetricasSLA:
    ...
    def calcular_sla_chamado(self, chamado, now: Optional[datetime] = None) -> Optional[Dict]:
        ...
        # Usa o `now` fixo passado pelo chamador para garantir consistência
        # durante o cálculo de todos os chamados na mesma requisição
        _now = now or datetime.utcnow()
    ...
    def obter_metricas_dashboard(
        ...
        # ✅ Captura o instante atual UMA única vez para toda a requisição.
        # Isso garante que todos os chamados sejam calculados com o mesmo
        # ponto de referência — eliminando a variação de valor a cada refresh.
        ...
        for c in chamados:
            # Passa _now fixo para que todos os chamados usem o mesmo instante
            s = self.calcular_sla_chamado(c, now=_now)
            if not s:
                continue
