...
class MetricsCalculator:
    ...
    def get_basic_metrics(db: Session, dias: int = 30) -> dict:
        ...
        abertos_agora = db.query(Chamado).filter(
            and_(Chamado.status == "Aberto", Chamado.deletado_em.is_(None))
        ).count()

        em_atendimento = db.query(Chamado).filter(
            and_(Chamado.status == "Em atendimento", Chamado.deletado_em.is_(None))
        ).count()

        aguardando = db.query(Chamado).filter(
            and_(Chamado.status == "Aguardando", Chamado.deletado_em.is_(None))
        ).count()
    ...
    def get_chamados_por_dia(db: Session, dias: int = 7, statuses: list = None) -> list:
        ...
        for i in range(dias - 1, -1, -1):
            ...
            row = {"dia": ini.strftime("%a"), "data": ini.strftime("%Y-%m-%d")}
            total = 0
            for st in all_statuses:
                count = db.query(Chamado).filter(and_(
                    Chamado.data_abertura >= ini,
                    Chamado.data_abertura < fim,
                    Chamado.deletado_em.is_(None),
                    Chamado.status == st
                )).count()
                key = st.lower().replace(" ", "_").replace("í", "i").replace("ú", "u").replace("ó", "o").replace("ã", "a").replace("é", "e").replace("ç", "c")
                row[key] = count
                total += count
    ...
    def get_chamados_por_dia_periodo(db: Session, start_date: str, end_date: str, statuses: list = None) -> list:
        ...
        while current <= fim_total:
            ...
            row = {"dia": current.strftime("%a"), "data": current.strftime("%Y-%m-%d")}
            total = 0
            for st in all_statuses:
                count = db.query(Chamado).filter(and_(
                    Chamado.data_abertura >= current,
                    Chamado.data_abertura < prox,
                    Chamado.deletado_em.is_(None),
                    Chamado.status == st
                )).count()
                key = st.lower().replace(" ", "_").replace("í", "i").replace("ú", "u").replace("ó", "o").replace("ã", "a").replace("é", "e").replace("ç", "c")
    ...
    def get_chamados_por_mes(db: Session, meses: int = 3, statuses: list = None) -> list:
        ...
        for i in range(meses - 1, -1, -1):
            ...
            for st in all_statuses:
                count = db.query(Chamado).filter(and_(
                    Chamado.data_abertura >= primeiro,
                    Chamado.data_abertura < fim,
                    Chamado.deletado_em.is_(None),
                    Chamado.status == st
                )).count()
