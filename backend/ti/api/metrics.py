...
def get_overview(
    ...
    """
    Endpoint unificado — retorna todos os dados da Visão Geral em uma única requisição.
    Evita os 5+ round-trips separados que causavam lentidão no carregamento.
    """
    from datetime import timedelta

    status_list = [s.strip() for s in statuses.split(",") if s.strip()] if statuses else []
    dias_map = {"7d": 7, "30d": 30, "90d": 90, "all": 365}
    semanas_map = {"7d": 2, "30d": 4, "90d": 13, "all": 52}
    meses_map = {"7d": 1, "30d": 3, "90d": 12, "all": 24}
    dias = dias_map.get(range, 30)
    ...
    try:
        ...
        if start_date and end_date:
            try:
                ...
                basic = MetricsCalculator.get_basic_metrics_periodo(db, data_inicio, data_fim) if hasattr(MetricsCalculator, "get_basic_metrics_periodo") else None
            except Exception:
                pass
        if basic is None:
            basic = MetricsCalculator.get_basic_metrics(db, dias) if hasattr(MetricsCalculator, "get_basic_metrics") else {}
    except Exception:
        basic = {}

    try:
        if start_date and end_date:
            daily = MetricsCalculator.get_chamados_por_dia_periodo(db, start_date, end_date, status_list or None)
        else:
            daily = MetricsCalculator.get_chamados_por_dia(db, dias, status_list or None)
        if not isinstance(daily, list): daily = []
    except Exception:
        daily = []

    try:
        if start_date and end_date:
            weekly = MetricsCalculator.get_chamados_por_semana_periodo(db, start_date, end_date, status_list or None)
        else:
            weekly = MetricsCalculator.get_chamados_por_semana(db, semanas_map.get(range, 4), status_list or None)
    ...
    try:
        if start_date and end_date:
            monthly = MetricsCalculator.get_chamados_por_mes_periodo(db, start_date, end_date, status_list or None)
        else:
            monthly = MetricsCalculator.get_chamados_por_mes(db, meses_map.get(range, 3), status_list or None)
        if not isinstance(monthly, list): monthly = []
    except Exception:
        monthly = []
    ...
    try:
        ...
        performance = {"tempo_resolucao_medio": "—", "primeira_resposta_media": "—", "taxa_reaberturas": "0%", "chamados_backlog": 0}
...
def get_basic_metrics(range: str = "30d", start_date: str = "", end_date: str = "", db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Abertos agora (sempre, não filtra por período) - apenas status "Aberto"
        ...
        # Aguardando agora (sempre, não filtra por período) - inclui Expirado (retroativos)
        aguardando_agora = db.query(Chamado).filter(
            and_(
                Chamado.status.in_(["Aguardando", "Expirado"]),
                Chamado.deletado_em.is_(None)
            )
        ).count()
