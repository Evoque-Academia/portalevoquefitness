...
# Cache simples em memória para o endpoint admin (evita queries repetidas em <5s)
import time as _time
_admin_cache: dict = {}
_ADMIN_CACHE_TTL = 5  # segundos
...
def listar_chamados(
    ...
    try:
        ...
        # Cache de curta duração para requisições admin idênticas (reduz carga no DB)
        if admin and not after_date:
            cache_key = f"admin:{limit}:{offset}"
        ...
        # Montar resultado usando o JOIN (sem N+1)
        result = []
