...
export function AuthProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      const sessionResponse = await fetch("/api/auth/session/create", {
        ...
        body: JSON.stringify({
          ...
          expires_in: 259200, // 3 days — alinhado com expiração do Auth0
        }),
      });
      ...
      // Store session token and user data in localStorage so they persist across tabs/browser restarts
      localStorage.setItem("auth_session_token", sessionData.session_token);
      localStorage.setItem("auth_expires_at", sessionData.expires_at);
      ...
      // Store user data in localStorage so it persists across tabs and browser restarts
      const authJson = JSON.stringify(userData);
      localStorage.setItem("evoque-fitness-auth", authJson);
      console.debug(
        "[AUTH] ✓ Saved to localStorage, length:",
        authJson.length,
      );
      console.debug(
        "[AUTH] Verify localStorage:",
        localStorage.getItem("evoque-fitness-auth") ? "OK" : "FAIL",
      );
  ...
  const checkExistingSession = async (): Promise<boolean> => {
    try {
      ...
      if (!response.ok) {
        console.error("[AUTH] Session validation request failed, status:", response.status, "- clearing session");
        localStorage.removeItem("auth_session_token");
        localStorage.removeItem("auth_expires_at");
        localStorage.removeItem("evoque-fitness-auth");
        setUser(null);
      ...
      if (validationData && !validationData.is_valid) {
        console.debug("[AUTH] Session is invalid per backend");
        localStorage.removeItem("auth_session_token");
        localStorage.removeItem("auth_expires_at");
        localStorage.removeItem("evoque-fitness-auth");
        setUser(null);
      ...
      const userDataRaw = localStorage.getItem("evoque-fitness-auth");
      if (userDataRaw) {
        try {
          ...
          console.error(
            "[AUTH] Failed to parse user data from localStorage:",
            e,
          );
          localStorage.removeItem("evoque-fitness-auth");
          setUser(null);
  ...
  const login = async (email?: string, password?: string) => {
    try {
      ...
      localStorage.setItem("evoque-fitness-auth", authJson);
      console.debug(
        "[AUTH] ✓ Password login - Saved to localStorage, length:",
        authJson.length,
      );
